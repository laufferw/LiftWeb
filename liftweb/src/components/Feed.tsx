...
export default function Feed() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const logRows = (logData ?? [])
        .filter((log) => !blockedIds.includes(log.user_id)) as RawLog[];
  ...
  const emptyState = useMemo(() => logs.length === 0, [logs.length]);
  ...
  if (emptyState) {
    return (
      <div className="rounded-3xl border border-border bg-surface p-6 text-sm text-muted shadow-card">
        No logs yet. Be the first to <Link href="/log/new" className="font-semibold text-ink">log a workout</Link>.
      </div>
    );
  }
